...
sns.set()
#%%导入相关的数据
new_data = pd.read_csv('E:\\Spyder_2022.3.29\\data\\machinel\\lwl_data\\OSA\\serve_osa_2.csv')
# 自变量和因变量的选择
X= new_data[["ESSL","hypertension","BQL","SBSL","drink",'smork',"snoring",'suffocate','memory','LOE','gender','age','BMI','waistline','NC']]
X_data = new_data[["ESSL","hypertension","BQL","SBSL","drink",'smork',"snoring",'suffocate','memory','LOE','gender','age','BMI','waistline','NC']]
X = np.array(X)
y = new_data[['OSAL']]
y = label_binarize(y, classes=[0,1])
random_state_new = 50
#%%划分数据集和数据不平衡的处理
X_train_raw, X_test, y_train_raw, y_test = train_test_split(X, y, random_state=random_state_new)
...
bag_model = BaggingClassifier(KNeighborsClassifier(),
                              n_estimators=10, 
                              *, 
                              bootstrap=True, 
...
gbm_model = GradientBoostingClassifier(n_estimators=100, 
                                       ...
                                       ccp_alpha=0.0,
                                       random_state=random_state_new)
xgb_model = xgb.XGBClassifier(n_estimators=360, 
                              max_depth=1, 
                              learning_rate=1.6, 
                              random_state=random_state_new)
...
#%%模型准确率的比较
print("Logistic回归的模型准确率：{:.3f}".format(lr_model.score(X_test, y_test)))
...
print("MLP模型的模型准确率：{:.3f}".format(mlp_model.score(X_test, y_test)))
#%%交叉验证
...
plt.plot(fpr_lr, tpr_lr, 
         label="ROC Curve LR; AUC = {:.3f}".format(roc_auc_lr))
plt.plot(fpr_ab, tpr_ab, 
         label="ROC Curve AB; AUC = {:.3f}".format(roc_auc_ab))
...
sen_rf, spc_rf = round(TP_rf/(TP_rf+FN_rf), 
                       3), round(TN_rf/(FP_rf+TN_rf), 3)
sen_lr, spc_lr = round(TP_lr/(TP_lr+FN_lr), 
                       3), round(TN_lr/(FP_lr+TN_lr), 3)
sen_dt, spc_dt = round(TP_dt/(TP_dt+FN_dt), 
                       3), round(TN_dt/(FP_dt+TN_dt), 3)
...
plt.show()
#%%绘制混淆矩阵train
gbm_prob_train = gbm_model.predict(X_train)
cm = confusion_matrix(y_train, gbm_prob_train)
...
plt.show()
#%%绘制混淆矩阵test
gbm_prob_test = gbm_model.predict(X_test)
cm = confusion_matrix(y_test, gbm_prob_test)
disp = ConfusionMatrixDisplay(confusion_matrix=cm, display_labels=['Low risk', 'High risk'])
sns.set_style("white")
disp.plot(cmap='RdPu')
...
#%%特征重要性
sns.set()
explainer = shap.Explainer(gbm_model, X_data)
